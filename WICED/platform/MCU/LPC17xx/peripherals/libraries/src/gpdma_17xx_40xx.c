...
volatile const void *GPDMA_LUTPerAddr[] = {
		...
		(&LPC_I2S->RXFIFO), 		// I2S Rx
		(&LPC_DAC->CR),				// DAC
		(&LPC_UART0->dll_thr_rbr.THR),	// UART0 Tx
		(&LPC_UART0->dll_thr_rbr.RBR),	// UART0 Rx
		(&LPC_UART1->dll_thr_rbr.THR),	// UART1 Tx
		(&LPC_UART1->dll_thr_rbr.RBR),	// UART1 Rx
		(&LPC_UART2->dll_thr_rbr.THR),	// UART2 Tx
		(&LPC_UART2->dll_thr_rbr.RBR),	// UART2 Rx
		(&LPC_UART3->dll_thr_rbr.THR),	// UART3 Tx
		(&LPC_UART3->dll_thr_rbr.RBR),	// UART3 Rx
		(&LPC_TIMER0->MR[0]),				// MAT0.0
		(&LPC_TIMER0->MR[1]),				// MAT0.1
		(&LPC_TIMER1->MR[0]),				// MAT1.0
		(&LPC_TIMER1->MR[1]),				// MAT1.1
