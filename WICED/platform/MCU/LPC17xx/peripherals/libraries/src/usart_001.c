...
void IP_UART_Init(IP_USART_001_T *pUART, IP_UART_ID_T UARTPort)
{
	...
	/* FIFOs are empty */
	pUART->fcr_iir.FCR = ( UART_FCR_FIFO_EN | UART_FCR_RX_RS | UART_FCR_TX_RS);
	/* Disable FIFO */
	// pUART->FCR = 0;
	/* Dummy reading */
	while (pUART->LSR & UART_LSR_RDR) {
		tmp = pUART->dll_thr_rbr.RBR;
	}
...
Status IP_UART_SetBaud(IP_USART_001_T *pUART, uint32_t baudrate, uint32_t uClk)
{
	...
	if (best_error < UART_ACCEPTED_BAUDRATE_ERROR) {
		...
		pUART->ier_dlm.DLM = UART_LOAD_DLM(best_divisor);
		pUART->dll_thr_rbr.DLL = UART_LOAD_DLL(best_divisor);
		/* Then reset DLAB bit */
...
Status IP_UART_SendByte(IP_USART_001_T *pUART, uint8_t data)
{
	...
	pUART->dll_thr_rbr.THR = data & UART_THR_MASKBIT;
	return SUCCESS;
}
...
Status IP_UART_ReceiveByte(IP_USART_001_T *pUART, uint8_t *Data)
{
	...
	*Data = (uint8_t) (pUART->dll_thr_rbr.RBR & UART_RBR_MASKBIT);
	return SUCCESS;
}
...
void IP_UART_IntConfig(IP_USART_001_T *pUART, IP_UART_INT_T UARTIntCfg, FunctionalState NewState)
{
	...
	if (NewState == ENABLE) {
		pUART->ier_dlm.IER |= tmp;
	}
	else {

		pUART->ier_dlm.IER &= (~tmp) & UART_IER_BITMASK;
...
{
	return (pUART->fcr_iir.IIR) & UART_IIR_BITMASK;
}
...
void IP_UART_FIFOConfig(IP_USART_001_T *pUART, UART_FIFO_CFG_T *FIFOCfg)
{
	...
	pUART->fcr_iir.FCR = tmp & UART_FCR_BITMASK;
}
