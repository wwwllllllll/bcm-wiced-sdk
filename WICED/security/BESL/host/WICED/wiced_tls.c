...
extern int32_t x509parse_crt( x509_cert *chain, unsigned char *buf, uint32_t buflen );
extern int32_t x509parse_crtfile( x509_cert *chain, char *path );
extern int32_t x509parse_key( rsa_context *rsa, unsigned char *buf, uint32_t buflen, unsigned char *pwd, uint32_t pwdlen );
...
besl_result_t wiced_tls_load_certificate( wiced_tls_certificate_t* certificate, const char* cert_string )
{
...
besl_result_t wiced_tls_load_key( wiced_tls_key_t* key, const char* key_string )
{
...
wiced_result_t wiced_tcp_start_tls( wiced_tcp_socket_t* socket, wiced_tls_endpoint_type_t type, wiced_tls_certificate_verification_t verification )
{
    ...
    wwd_wifi_get_random((uint16_t*)&rngstate.entropy);
    wwd_wifi_get_random(((uint16_t*)&rngstate.entropy)+1);
...
wiced_result_t wiced_tls_receive_packet(wiced_tcp_socket_t* socket, wiced_packet_t** packet, uint32_t timeout)
{
    ...
    else
    {
        ...
        else
        {
            ...
            while ( tls_get_next_record( context, &temp_record, 0 ) == WICED_SUCCESS )
            {
...
static wiced_result_t tls_packetize_buffered_data( wiced_tls_context_t* context, wiced_packet_t** packet )
{
    ...
    result = wiced_packet_create_tcp( context->send_context, MIN(1024, record_length - context->defragmentation_buffer_bytes_processed), (wiced_packet_t**) packet, &data, &length );
    if ( result  != WICED_SUCCESS )
    {
        *packet = NULL;
        return result;
