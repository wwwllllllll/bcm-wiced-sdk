...
static const char base64_normal[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
int base64_encode( unsigned char const* src, uint32_t src_length, unsigned char* target, uint32_t target_size, base64_encode_options_t options )
{
    ...
    if ( ( options & BASE64_ENC_URL_SAFE_CHARSET ) == 0 )
    {
        base64 = base64_normal;
    }
    else
    {
        base64 = base64_url;
    ...
    if ( ( src_length != 0 ) && ( ( options & BASE64_ENC_NO_PADDING ) == 0 ) )
    {
        /* Get what's left. */
...
int base64_decode( unsigned char const* src, uint32_t src_length, unsigned char* target, uint32_t target_size, base64_decode_options_t options )
{
    ...
    if ( options != BASE64_DEC_URL_SAFE_CHARSET )
    {
        base64 = base64_normal;
    }
    else
    ...
    if ( ( ch == pad64 ) && ( ( src_length < 0 ) || ( src - orig_src < src_length ) ) )
    {
